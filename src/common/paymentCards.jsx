...
const PaymentCards = (props) => {
  ...
  return (
    <div className='justContCenter flexRow display margTop40'>
      {payment_plans.map((pym, index) => {
        return (
          <div
            className='select-cards-payment'
            key={index}
            style={{ padding: '1.7rem' }}
          >
            <div className='justContStart flexRow display'>
              <button className={`${pym.class}`}>
                <span style={{ color: '#FEFEFE' }}>{pym.type}</span>
              </button>
            </div>
            <div className='justContStart flexRow display'>
              <h2>
                {pym.title.includes('$') ? (
                  ...
                  <span className='plan-head'>{pym.title}</span>
                )}
              </h2>
            </div>
            <span className='plan-month'>Features:</span>
            <div className='justContStart flexCol display'>
              {planFeatures(pym.desc)}
            </div>
            {pym.title.includes('$') ? (
              <button
                className='plan-button'
                onClick={() => handleSubscription(pym)}
              >
                Subscribe
              </button>
            ) : (
              <button className='plan-button-custom'>
                <span style={{ color: '#FEFEFE' }}>Contact Us</span>
              </button>
            )}
          </div>
        );
