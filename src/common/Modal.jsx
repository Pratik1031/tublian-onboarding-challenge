...
const Modal = (props) => {
  ...
  const [mode, setMode] = useState([
    { id: 23, modeType: 1, checked: false },
    { id: 32, modeType: 2, checked: false },
  ]);
        ...
        <div className='plan-details'>
          <div className='plan-details-mainhead'>
            ...
            <p
              ...
              style={{ color: '#FDD649', cursor: 'pointer' }}
              onClick={props.onClose}
            >
              Change plan
            </p>
          ...
          <div className='display flex'>
            <div className='justContStart flexRow display'>
              <button className={`${props.preference.class} marginLeft12`}>
                <span style={{ color: '#FEFEFE' }}>{type}</span>
              </button>
            </div>
            {/* {type} */}
          </div>
          <div className='plan-details-mainhead'>
            ...
            <p className='payment-change'>
              <span className='currstyles'>USD</span>
              {title}
              <span className='currstyles'>/Month</span>
            </p>
          </div>
          ...
          <div className='payment-options-select'>
            <ul className='payment-lists'>
              <li
                className={
                  mode[0].checked ? 'list-select-checked' : 'list-select'
                }
              >
                <input
                  type='checkbox'
                  ...
                  onChange={() => {
                    handleCheckboxChange(mode[0].id);
                  }}
                />
                <label htmlFor={`checkbox-credit`}>
                  {' '}
                  <span className='flexRow display'>
                    <img src={card} alt='credit-card' className='credit-card' />
                    Credit Card
                  </span>
                </label>
              </li>
              <li
                className={
                  mode[1].checked ? 'list-select-checked' : 'list-select'
                }
              >
                <input
                  type='checkbox'
                  ...
                  onChange={() => {
                    handleCheckboxChange(mode[1].id);
                  }}
                />
                <label htmlFor={`checkbox-gpay`}>
                  {' '}
                  <span className='flexRow display'>
                    <img src={gpay} alt='goole-card' className='google-card' />
                  </span>
                </label>
              </li>
            </ul>
          ...
          <div className='flexCol flex display'>
            ...
            <div
              className='plan-details-mainhead'
          ...
          <p className='marginBotttom12'>
            By clicking below, you agree to our{' '}
            <span className='textColorYellow'>Terms</span>,
            <span className='textColorYellow'>Privacy Policy</span> and{' '}
            <span className='textColorBlue'>Automatic Renewal</span>. Tublian
            will charge you $49.99 (plus Tax) each month until you cancel you
            subscription in account settings.
          </p>
          <button
            className='payment-process-sucess'
            onClick={() => props.setSucess(true)}
          >
            Pay {title}
          </button>
        </div>
...
<img src={stripe1} alt="logo" className="payment-brand"/>
    <li className='marginLeft12 margTop10'>Powered by Stripe.com</li>
      ...
      <div
        className='display flexRow justContStart'
        style={{ width: '540px', position: 'absolute', bottom: 0 }}
      >
        <img src={stripe1} alt='logo' className='payment-brand' />
        <li className='marginLeft12 margTop10'>Powered by Stripe.com</li>
      </div>
